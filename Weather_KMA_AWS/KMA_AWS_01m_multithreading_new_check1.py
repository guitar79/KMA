...
class check_csvfile():
    ...
    def fetch(self):
        ...
        if len(row) < 3 or len(row) > 100 : er = 100
        elif row[6]=='.' : er = er + 1
        if er > 40 :
	...
	def run(self):
		for Ho in range(0,24):
			for Mn in range(0,60):
		                #fetcher_check = check_csvfile(self.year, self.month, self.day, Ho, Mn, self.threadno)
		                #fetcher_check()
		                fetcher_crawler = crawler(self.year, self.month, self.day, Ho, Mn, self.threadno)
